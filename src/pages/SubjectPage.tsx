...
import { SubjectMockTest, Subject } from '../types';
...
export function SubjectPage() {
  ...
  const [logs, setLogs] = useState<SubjectMockTest[]>([]);
  ...
  const subjectTitle = subjectName ? subjectName.charAt(0).toUpperCase() + subjectName.slice(1) : '';
  ...
  const fetchLogs = async () => {
    ...
    try {
      const q = query(
        collection(db, `users/${user.uid}/subjectMockTests`),
        orderBy('testDate', 'asc')
      );
      const snapshot = await getDocs(q);
      const data = snapshot.docs
        .map(doc => doc.data() as SubjectMockTest)
        .filter(log => log.subject.toLowerCase() === subjectName?.toLowerCase());
      setLogs(data);
    } catch (error) {
      handleFirestoreError(error, OperationType.GET, `users/${user.uid}/subjectMockTests`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const percentage = maxScore === 0 ? 0 : parseFloat(((score / maxScore) * 100).toFixed(2));
    
    const testId = Date.now().toString();
    const newLog: SubjectMockTest = {
      id: testId,
      userId: user.uid,
    ...
    try {
      await setDoc(doc(db, `users/${user.uid}/subjectMockTests`, testId), newLog);
      ...
      handleFirestoreError(error, OperationType.WRITE, `users/${user.uid}/subjectMockTests/${testId}`);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!user) return;
    try {
      await deleteDoc(doc(db, `users/${user.uid}/subjectMockTests`, id));
      setLogs(logs.filter(log => log.id !== id));
    } catch (error) {
      handleFirestoreError(error, OperationType.DELETE, `users/${user.uid}/subjectMockTests/${id}`);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#0A0A0B]">
      <header className="h-20 border-b border-white/5 flex items-center justify-between px-10 shrink-0">
        <div>
          <h2 className="text-xl font-serif italic text-white">{subjectTitle}</h2>
          <p className="text-xs text-slate-500 uppercase tracking-widest">Performance Trend</p>
        </div>
        <button
        ...
        <div className="flex-1 flex flex-col gap-8">
          {logs.length > 0 ? (
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="percentage" stroke="#6366f1" strokeWidth={2} dot={{ fill: '#6366f1', strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-[#111114] p-12 text-center rounded-2xl border border-white/5 border-dashed">
              <p className="text-slate-500 text-sm">No mock test logs yet. Add one to see your progress graph!</p>
            </div>
          )}
        </div>
                ...
                <li key={log.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:bg-white/5 transition-colors">
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500 mt-1 uppercase tracking-wider flex space-x-3">
                      ...
                      <span className="text-red-500">✖ {log.wrongAnswers}</span>
                      <span className="text-indigo-400">Score: {log.score || 0}</span>
                    </div>
                  </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-[#0A0A0B]/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[#111114] rounded-2xl border border-white/5 shadow-2xl max-w-md w-full p-8">
            <h2 className="text-xl font-serif italic text-white mb-6">Add Log Entry</h2>
            <form onSubmit={handleCreate} className="space-y-5">
              ...
              <div className="flex items-center space-x-2 pt-2">
                <input type="checkbox" id="negMarking" checked={hasNegative} onChange={e => setHasNegative(e.target.checked)} className="rounded border-none bg-[#1A1A1D] checked:bg-indigo-500" />
                <label htmlFor="negMarking" className="text-sm text-slate-300">Enable Negative Marking (-1 for wrong)</label>
              </div>
